        ...
        FB.ui({
              ...
              function(response) {                                       
                    ...
                    {                                               
                        ...
                        {
                            $('#paymentDiv').html('');
                            $('#paymentDiv').html('<div style="display:block;height:100%" id="paymentScreen" class="pop-full"><div style="display: block; text-align: center; margin-top: 15%;"><img src="/fb_app/images/ajax-loader.gif"></div></div>');
                        }
...
{
    var PAY_SERVER = SERVER.substr(0,(SERVER.length-3));
    DataCall("status"+response.status+"&signed_req="+response.signed_request+"&qty="+response.quantity+"&paymentid="+response.payment_id+"&videoid="+VIDEO_ID, PAY_SERVER + "fb_local_currency_payment.json/", "POST", true,
...
$('#description_anchor').live('click', function(){
    for(i=0;i<3;i++) {
        $('div.desc-container').fadeTo('slow', 0.5).fadeTo('slow', 1.0);
    }
});
