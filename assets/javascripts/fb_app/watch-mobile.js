...
function LoadHTMLPlayer(poster, video, title, buy) {
    ...
    if (isApple || isAndroid) {
        player = player + '<video id="vdo-player"  poster="'+poster+'" class="projekktor" title="' + title + '" width="100%" height="' + disp_height + '" controls>';
    }
    else
        player = player + '<video id="vdo-player"  poster="'+poster+'" class="projekktor" title="' + title + '" width="100%" height="' + disp_height + '" controls>';
    //IF NOT IE
    ...
    var controlsTemplate = '<div class="play-bar-wrap"><div class="play-bar-wraper"><div %{scrubber}><div %{loaded}></div><div %{playhead}></div><div %{scrubberdrag}></div></div></div></div><div class="control-wraper clearfix"><ul class="play fl"><li><a href="#" onclick="return false;"><div %{play} onclick="playBtn();"></div><div %{pause}></div></a></li><li id="vBar"><span class="ppmute" id="vmute" onclick="vToggle(1);"></span><span onclick="vToggle(2);" style="display:none" id="vmax" class="ppvmax"></span></li></ul><div class="left-section clearfix"><ul class="link icos fl"><li><span style="display:none;" class="btnpreview" id="previewBtn" onclick="btnPreview();">Preview</span></li><li><span class="btnabout" id="aboutBtn" onclick="btnAbout();">About</span></li></ul></div><div class="right-section clearfix"><ul class="link icos fl"><li><div %{timeleft}>%{hr_elp}:%{min_elp}:%{sec_elp} <span class="grey">/ %{hr_dur}:%{min_dur}:%{sec_dur}</span></div></li><li><div %{fsexit}></div><div %{fsenter}></div></li><li id="hdWrapper"><a href="#"><span id="hdsdswitch" class="hdsd" onclick="dispHdSd();"></span></a><span style="display:none" id="hdSpanBar"><ul><li><a href="#" id="hdplug" onclick="hdswitch();">720p</a></li><li><a id="sdplug" href="#" onclick="sdswitch();">480p</a></li></ul></span></li><div class="player-logo"><span class="logo"></span></div>';
    ...
    if (buy)
        if (/Android|ipad|iPhone/i.test(navigator.userAgent)) {
                controlsTemplate = controlsTemplate + '<div class="button" id="player-buy-button" ><a rel="noreferrer" href="https://m.facebook.com/apps/' + FB_APP_NAME + '/?watch=' + VIDEO_ID + '" >Buy Now</a></div>';
            }else{
                controlsTemplate = controlsTemplate + '<div class="button" id="player-buy-button" ><a href="#" onclick="startTOC();">Buy Now</a></div>';
        }
    ...
    function (data) {
        try {
            ...
            */
            ...
            var pubName = (getParameterByName('page_name') || getParameterByName('publisher'));
            if (pubName==""){
                ...
                var pageLink = (getParameterByName('page_id') || getParameterByName('from_page_id'));
            ...
            projekktor('#vdo-player').addListener('time', previewLogListner);
            StreamMedia(false,
...
previewLogListner = function(value){
    if( value > 3 )
        logView(value)
    if( value > 9 && !ACTIVITY_CALL_SENT ){
        sendPreviewCall();
        ACTIVITY_CALL_SENT = true
